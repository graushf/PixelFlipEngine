...
// bytes. The memory pool will allocate the appropiate amount of memory and set up the data
// structure in the Init() call. Thus, total memory usage will be N * (S + 4) + O, where N is the
// number of chunks, S is the size of each chunk, and O is the overheadd for the class (currently
...
// will cause the chunk to be inserted to the front of the list, ready for the next bit.
//---------------------------------------------------------------------------------------------------
class MemoryPool
{
	...
	unsigned int m_chunkSize, m_numChunks;			// the size of each chunk and the number of chunks per array, respectively
